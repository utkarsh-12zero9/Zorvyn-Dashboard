...
export function AdvancedInsights() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          ...
          {timeData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={timeData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                ...
                />
                <Area type="monotone" dataKey="cumulativeSavings" stroke="#3b82f6" strokeWidth={3} fillOpacity={1} fill="url(#colorSavings)" isAnimationActive={true} />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
      ...
      {/* Cashflow Breakdowns */}
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 transition-[transform,box-shadow,border-color,background-color] duration-300 transform transform-gpu hover:-translate-y-1 hover:shadow-lg hover:shadow-purple-500/10 dark:hover:shadow-purple-500/10 hover:border-purple-500/30 group">
        <h3 className="text-lg font-semibold text-zinc-900 dark:text-white mb-2 transition-colors duration-300">Monthly Cashflow Dynamics</h3>
        <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-6 transition-colors duration-300">Income vs Expenses overlaid with Net flow.</p>
        <div className="h-80">
          {timeData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={timeData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                ...
                <Bar dataKey="income" name="Income" fill="#22c55e" radius={[4, 4, 0, 0]} barSize={20} isAnimationActive={true} />
                <Bar dataKey="expense" name="Expense" fill="#f97316" radius={[4, 4, 0, 0]} barSize={20} isAnimationActive={true} />
                <Line type="monotone" dataKey="net" name="Net Flow" stroke="#a855f7" strokeWidth={3} dot={{ r: 4, fill: '#a855f7', strokeWidth: 2, stroke: '#18181b' }} isAnimationActive={true} />
              </ComposedChart>
            </ResponsiveContainer>
          ) : (
             <div className="h-full flex items-center justify-center text-zinc-500">No data available</div>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 relative transition-[transform,box-shadow,border-color,background-color] duration-300 transform transform-gpu hover:-translate-y-1 hover:shadow-lg hover:shadow-red-500/10 dark:hover:shadow-red-500/10 hover:border-red-500/30 group">
