...
export function Insights() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div className="bg-zinc-900/80 backdrop-blur-md border border-zinc-800 rounded-xl p-5 flex items-start gap-4">
        <div className="p-2.5 bg-red-400/10 border border-red-500/20 rounded-lg text-red-400">
          <Flame size={18} />
        </div>
        <div>
          <h4 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Top Spending</h4>
          <p className="text-lg font-bold text-white mt-1 leading-snug">{insights.topCategory}</p>
      ...
      <div className="bg-zinc-900/80 backdrop-blur-md border border-zinc-800 rounded-xl p-5 flex items-start gap-4">
        ...
        <div>
          <h4 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Monthly Avg</h4>
          <p className="text-lg font-bold text-white mt-1 leading-snug">${insights.avgMonthlyExpense.toLocaleString('en-US', {maximumFractionDigits: 0})}</p>
      ...
      <div className="bg-zinc-900/80 backdrop-blur-md border border-zinc-800 rounded-xl p-5 flex items-start gap-4">
        ...
        <div>
          <h4 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Largest Purchase</h4>
          <p className="text-lg font-bold text-white mt-1 leading-snug">${insights.maxSingleExpense.amount.toLocaleString()}</p>
          <p className="text-xs text-zinc-500 mt-1">{insights.maxSingleExpense.category}</p>
        </div>
      </div>

      {/* 4. Financial Health / Savings Rate */}
      <div className={`bg-zinc-900/80 backdrop-blur-md border rounded-xl p-5 flex items-start gap-4 transition-colors ${insights.isOverspending ? 'border-red-500/30' : 'border-zinc-800'}`}>
        ...
        <div>
          <h4 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Savings Rate</h4>
          <p className="text-lg font-bold text-white mt-1 leading-snug">{insights.savingsRate}%</p>
